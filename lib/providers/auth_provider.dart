...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> register(String email, String password, String name, int age, String userTypes) async {
    try {
 ...
 Future<void> sendEmailDirectlyViaSendGrid(String email, String code) async {
  ...
  final emailPayload = {
    ...
    "content": [
      {
        "type": "text/plain",
        "value": "Your verification code is: $code. It will expire in 5 minutes."
      }
    ]
  };
...
//Sign in using Google
 Future<String> signInWithGoogle() async {
  try {
    ...
    final credential = GoogleAuthProvider.credential( // Create a credential using the Google authentication details
      idToken: googleAuth.idToken,
      accessToken: googleAuth.accessToken,
    );

    final userCredential = await _auth.signInWithCredential(credential); // Sign in to Firebase using the Google credential

    final user = userCredential.user; // Check if the user is not null
    if (user != null) { 
      final doc = await _firestore.collection('users').doc(user.uid).get(); 
      if (!doc.exists) { // If the user document does not exist, create it
